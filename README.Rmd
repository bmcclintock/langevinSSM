...
To include barriers to movement (e.g., land for marine animals), the `prepBarrier` function can be used to create a signed distance field (SDF) from a binary raster mask (where 1 indicates allowed movement areas and 0 indicates restricted movement areas). The SDF is then included as a habitat selection covariate (where negative coefficients indicate attraction to the barrier boundary, e.g., the coast for marine animals) and as part of a penalty term with strength `lambda`:
```{r barrier, eval=TRUE}
