...
initialInner <- TRUE # logical indicating whether or not to use an initial inner optimization for the random effects (``mu'' and/or ``v_mu'') before optimizing over all parameters
...
for(isim in 1:nsims){
  ...
  if(model=="underdamped"){
    #init.v_mu <- subDat[[isim]][,c("v.x","v.y")] # true values
    init.v_mu <- matrix(0,nrow(subDat[[isim]]),2)
    if(!useInitialValues) par$v_mu = init.v_mu
  }
