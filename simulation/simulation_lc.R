...
tau <- c(1,1) # x- and y-axis error scaling parameter
rho_o <- 0 # error correlation
...
measurementError <- getEMF() # setting measurementError <- NULL adds no measurement error to observations

## specify scale and spatial autocorrelation for covariates
sca <- 600 # bounding box scale
...
if(model=="overdamped"){
  colnames(parMat) <- c(paste0("beta",1:(ncov+1)),"sigma","tau1","tau2","rho_o","BA")
} else colnames(parMat) <- c(paste0("beta",1:(ncov+1)),"sigma","gamma","tau1","tau2","rho_o","BA")
...
set.seed(1, kind="Mersenne-Twister", normal.kind = "Inversion")
for(isim in 1:nsims){
  subDat[[isim]] <- addMeasurementError(langSim[[isim]], par=list(tau=tau, rho_o=rho_o), measurementError = measurementError)
}
...
for(isim in 1:nsims){
  ...
  parMat[isim,"tau1"] <- langFit[[isim]]$estimates$natural["tau_1",1]
  parMat[isim,"tau2"] <- langFit[[isim]]$estimates$natural["tau_2",1]
...
save(parMat,beta,sigma,gamma,obsPerAnimal,subDat,langFit,tau,rho_o,timeStep,samplingRate,propMissing,measurementError,sca,npoints,curweight,covRange,map,covTimes,file=paste0("simulation/results/",dataName,"_tau",paste0(tau,collapse = "_"),"_rho_o",rho_o,"_samplingRate",samplingRate,"_propMissing",propMissing,"_aniMotum",use_aniMotum,"_npoints",npoints,"_curweight",curweight,"_zetaScale",zetaScale,".RData"))
