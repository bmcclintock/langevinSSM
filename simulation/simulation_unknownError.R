...
tau <- c(1,1) # x- and y-axis error scaling parameters
rho_o <- 0 # x- and y-axis error correlation parameter
...
x.sd <- 1.5 # SD for x-axis; x.err ~ abs(Normal(0,x.sd))
y.sd <- 1.5 # SD for semi-minor error ellipse axis; y.err ~ abs(Normal(0,y.sd))
measurementError <- list(x.sd = x.sd, y.sd = y.sd) # setting measurementError <- NULL adds no measurement error to observations
...
spatialCovs <- list()
parMat <- matrix(NA,nrow=nsims,9+ifelse(model=="underdamped",1,0))
if(model=="overdamped"){
  colnames(parMat) <- c(paste0("beta",1:(ncov+1)),"sigma",paste0("tau",1:2),"rho_o","BA")
} else colnames(parMat) <- c(paste0("beta",1:(ncov+1)),"sigma","gamma",paste0("tau",1:2),"rho_o","BA")
...
if(!file.exists(paste0("simulation/data/",dataName,".RData"))){
  for(isim in 1:nsims){
    ...
    if(model=="underdamped"){
      par <- list(beta=beta,sigma=sigma,gamma=gamma,tau=tau,rho_o=rho_o)
    } else {
      par <- list(beta=beta,sigma=sigma,tau=tau,rho_o=rho_o)
    }
...
emf_val <- getEMF()
set.seed(1, kind="Mersenne-Twister", normal.kind = "Inversion")
subDat <- lapply(1:nsims, function(isim) {

  dat <- addMeasurementError(langSim[[isim]], par = par, measurementError = measurementError)

  dat$x.err <- NA
  dat$y.err <- NA
  dat$lc <- "G"

  dat$date <- start_time + (dat$date * 3600)
  dat$eor <- dat$eor * 180 / pi
  class(dat) <- "data.frame"
  attr(dat, "time.unit") <- NULL
  suppressWarnings(formatData(dat, emf = emf_val))
})
...
for(isim in 1:nsims){
  ...
  par <- suppressMessages(initialValues(subDat[[isim]],model,par=par,spatialCovs=spatialCovs[[isim]]))
  ...
  parMat[isim,1:(8+ifelse(model=="underdamped",1,0))] <- langFit[[isim]]$par
  ...
  parMat[isim,paste0("tau",1:2)] <- langFit[[isim]]$estimates$natural[paste0("tau_",1:2),1]
  parMat[isim,"rho_o"] <- langFit[[isim]]$estimates$natural["rho_o",1]
...
save(parMat,beta,sigma,gamma,obsPerAnimal,subDat,langFit,tau,rho_o,timeStep,samplingRate,propMissing,measurementError,sca,npoints,curweight,covRange,map,useInitialValues,covTimes,file=paste0("simulation/results/",dataName,"_tau",paste0(tau,collapse="_"),"_rho_o",rho_o,ifelse(!is.null(measurementError),paste0("_x.sd",measurementError$x.sd,"_y.sd",measurementError$y.sd),""),"_samplingRate",samplingRate,"_propMissing",propMissing,"_aniMotum",use_aniMotum,"_npoints",npoints,"_curweight",curweight,"_zetaScale",zetaScale,".RData"))
