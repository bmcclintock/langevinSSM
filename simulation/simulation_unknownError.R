...
dataName <- paste0(model,"_nbAnimals",nbAnimals,"_obsPerAnimal",obsPerAnimal,"_timeStep",timeStep,"","_beta",paste0(beta,collapse = "_"),"_sigma",sigma,"_gamma",gamma,"_tau",paste0(tau,collapse="_"),"_rho_o",rho_o,"_sca",sca,"_covRange",paste0(covRange,collapse="_"),
...
subDat <- lapply(1:nsims, function(isim) {
  ...
  dat <- addMeasurementError(langSim[[isim]], measurementError = measurementError)
...
for(isim in 1:nsims){
  ...
  if(useInitialValues) par <- initialValues(subDat[[isim]],model,par=list(tau=tau,rho_o=rho_o),spatialCovs=spatialCovs[[isim]])
...
save(parMat,beta,sigma,gamma,obsPerAnimal,subDat,langFit,tau,rho_o,timeStep,samplingRate,propMissing,measurementError,sca,npoints,curweight,covRange,map,useInitialValues,covTimes,file=paste0("simulation/results/",dataName,ifelse(!is.null(measurementError),paste0("_x.sd",measurementError$x.sd,"_y.sd",measurementError$y.sd),""),"_samplingRate",samplingRate,"_propMissing",propMissing,"_aniMotum",use_aniMotum,"_npoints",npoints,"_curweight",curweight,"_zetaScale",zetaScale,".RData"))
