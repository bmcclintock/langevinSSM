...
test_that("tuneBarrier fails fast on invalid data classes", {
  ...
  expect_error(
    tuneBarrier(bad_df, mock_covs, barrier = "coast_mask", timeStep = "1 hour"),
...
test_that("tuneBarrier fails fast on insufficient data for empirical calculations", {
  ...
  expect_error(
    suppressMessages(tuneBarrier(tiny_df, mock_covs, barrier = "coast_mask", lambda_max = NULL, timeStep = "1 hour")),
    "Baseline 0 fit failed"
  )
})
...
test_that("tuneBarrier handles spatial overlap failure gracefully", {
  ...
  expect_error(
    suppressMessages(tuneBarrier(oob_df, mock_covs, barrier = "coast_mask", lambda_max = NULL, timeStep = "1 hour")),
...
test_that("tuneBarrier strictly enforces minimum data size even if lambda_max is provided", {
  ...
  expect_error(
    suppressMessages(tuneBarrier(tiny_df, mock_covs, barrier = "coast_mask", lambda_max = 50, n_coarse = 2, n_sims = 1, timeStep = "1 hour")),
    regexp = "All models in the coarse grid failed"
  )
})
...
test_that("tuneBarrier handles total optimization failure gracefully", {
  ...
  expect_error(
    suppressMessages(tuneBarrier(mock_df, mock_covs, barrier = "coast_mask",
                                 lambda_max = 1e6, n_coarse = 2, n_fine = 1, n_sims = 1, timeStep = "1 hour")),
...
test_that("Initialization grids calculate safely without throwing internal NA/NaN warnings", {
  expect_warning(
    tryCatch(
      suppressMessages(tuneBarrier(mock_df, mock_covs, barrier = "coast_mask",
                                   n_coarse = 2, n_fine = 1, n_sims = 1, timeStep = "1 hour")),
      error = function(e) NA
    ),
    regexp = NA
  )
...
test_that("tuneBarrier fails fast if no barrier attribute is found", {
  ...
  expect_error(
    tuneBarrier(mock_df, untagged_covs, barrier = "coast_mask", timeStep = "1 hour"),
    "barrier is not a 'barLangevin' object created by prepBarrier"
  )
})

test_that("tuneBarrier fails fast if barrier name not found in spatialCovs", {
  # Pass a string for a raster that isn't in the list
  expect_error(
    tuneBarrier(mock_df, mock_covs, barrier = "missing_mask", timeStep = "1 hour"),
