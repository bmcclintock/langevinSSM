...
library(TMB)
compile("src/underdampedLangevin.cpp")
dyn.load(dynlib("src/underdampedLangevin"))
...
gamma <- 0.25 # friction parameter (smaller value -> more directional persistence)
...
covlist <- list()
parMat <- matrix(NA,nrow=nsims,8,dimnames = list(NULL,c("sigma",paste0("beta",0:(ncov+1)),"gamma","UDcor")))
...
for(isim in 1:nsims){
  ...
  while(inherits(langSim[[isim]],"error")){
    ...
    langSim[[isim]] <- tryCatch(simulate_udLangevin_cpp(
        nbAnimals = nbAnimals,
  ...
  parMat[isim,1:7] <- langTMB[[isim]]$par
  parMat[isim,"sigma"] <- exp(parMat[isim,"sigma"] + log(scale_factor))
  parMat[isim,"gamma"] <- exp(parMat[isim,"gamma"])
...
save(parMat,beta,sigma,gamma,obsPerAnimal,langSim,langTMB,covlist,psi,lambda,propMissing,errorProp,scale_factor,resol,sca,file=paste0("data/underdampedLangevinSim_nbAnimals_",nbAnimals,"_obsPerAnimal_",obsPerAnimal,"_lambda_",lambda,"_sd_",sigma,"_gamma_",gamma,"_beta_",paste0(beta,collapse="_"),"_propMissing_",propMissing,"_errorProp_",errorProp,"_psi_",psi,".RData"))
